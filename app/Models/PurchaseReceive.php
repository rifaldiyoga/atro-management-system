...
class PurchaseReceive extends Model
{
  ...
  protected static function booted()
  {
    static::saving(function ($model) {
      if (empty($model->trxno) || $model->trxno === 'AUTO') {
        $model->trxno = TransactionHelper::generateTrxNo(static::class, 'PRCV', $model->trxdate);
      }
    });
  }
  ...
  {
    return $this->hasMany(PurchaseReceiveDetail::class, 'prcv_id', 'id');
  }
  ...
  {
    return $this->hasMany(Attachment::class, 'refid', 'id')->where('reftype', 'PRCV');
  }
}
